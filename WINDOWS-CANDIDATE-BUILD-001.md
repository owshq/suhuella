...
This candidate is `22a2c0b` replayed onto `3d0f05c` (the commit that produced the public installer). Site dictionary copy is out of scope. `origin/windows-lifecycle-candidate-001` stays on the unbuildable `main` history; do not force-push it.
...
```bash
git checkout windows-lifecycle-candidate-002
...
gh workflow run desktop-windows-build.yml \
  --ref windows-lifecycle-candidate-002 \
  -f git_ref=windows-lifecycle-candidate-002 \
